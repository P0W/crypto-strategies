...
impl BinanceClient {
    ...
    pub async fn get_klines(
        ...
        let limit = limit.unwrap_or(MAX_KLINES_PER_REQUEST).min(MAX_KLINES_PER_REQUEST);
        params.push(("limit", limit.to_string()));
        
        debug!("Fetching klines: symbol={}, interval={}, limit={}", symbol, interval, limit);
    ...
    pub async fn get_server_time(&self) -> Result<DateTime<Utc>> {
        ...
        DateTime::from_timestamp_millis(time_resp.server_time)
            .context("Invalid server time")
    }
}
